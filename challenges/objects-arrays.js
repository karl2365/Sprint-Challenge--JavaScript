...
let universities = [];
universities = graduates.map((g) => g.university);
universities.sort()
...
let contactInfo = [];
contactInfo = graduates.map((g) => [g.first_name, g.email]);
for (let i=0;i<contactInfo.length;i++){
  console.log(`${contactInfo[i][0]} ${contactInfo[i][1]}`)
}
...
let lowerCase = [];
lowerCase = zooAnimals.map((z) => z.animal_name.toLowerCase());
...
largerPopulation = zooAnimals.filter((lp) => lp.population < 5);
...
*/
let populationTotal = zooAnimals.reduce(function(acc, zoo){
  acc += zoo.population;
  return acc;
},0);
